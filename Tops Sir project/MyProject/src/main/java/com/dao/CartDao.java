...
public class CartDao {
	...
	{
		System.out.println(c.getProduct_price());
		try {
	...
	public static List<Cart> getCartByUser(int uid)
	{
		...
		try {
			...
			{
				Cart cart=new Cart();
				cart.setCid(rSet.getInt("cid"));
				cart.setPid(rSet.getInt("pid"));
				cart.setUid(rSet.getInt("uid"));
				cart.setProduct_price(rSet.getInt("product_price"));
				cart.setProduct_qty(rSet.getInt("product_qty"));
				cart.setTotal_price(rSet.getInt("total_price"));
				cart.setPayment_status(rSet.getString("payment_status"));
				list.add(cart);
			}
		} catch (Exception e) {
			e.printStackTrace();
